...
data modify storage open: Tmp[].detail.MaxHealth set value 0.01
execute store result storage open: Tmp[].detail.MaxHealth float 0.01 run scoreboard players get @s ExWeapons.rnd
item modify entity @s weapon.mainhand exweapons:lore_nbt/copy_lore_nbt
data modify storage open: Set.Rng set from storage open: Tmp[].detail.MaxHealth
scoreboard players set #check ExWeapons.rnd -1
# item modify entity @s weapon.mainhand exweapons:lore_nbt/status/detail/health