...
scoreboard players add @s ExWeapons.rnd 170
data modify storage open: Tmp[].detail.Mul_Armor set value 0.1
execute store result storage open: Tmp[].detail.Mul_Armor float 0.1 run scoreboard players get @s ExWeapons.rnd
item modify entity @s weapon.mainhand exweapons:lore_nbt/copy_lore_nbt
data modify storage open: Set.Rng set from storage open: Tmp[].detail.Mul_Armor
