# 読み込み時の値適応-Armor
## 記録
scoreboard players add $CommandPositionSet_Armor ExWeapons.rng 1
## リセット
attribute @s generic.armor modifier remove 6D-1-9CB1-0-1
## 結合-計算
execute store result storage exweapons: command.rng float 0.001 run scoreboard players get @s ExWeapons.DetailAbility.ArmorMul
...
data merge storage exweapons: {command:{ability:["attribute ","player_name"," generic.armor modifier add 6D-1-9CB1-0-1 Ability_ArmorMul ","0","0","0",".","0","0","0"," multiply"]}}
...
scoreboard players operation $CommandPosition ExWeapons.rng = $CommandPositionSet_Armor ExWeapons.rng
execute positioned 13403 -64 13401 run function exweapons:core/other/concat/position_set