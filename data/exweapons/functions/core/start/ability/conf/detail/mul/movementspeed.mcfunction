# 読み込み時の値適応-MovementSpeed
## 記録
scoreboard players add $CommandPositionSet_MovementSpeed ExWeapons.rng 1
## リセット
attribute @s generic.movement_speed modifier remove 70-1-9CB1-0-1
## 結合-計算
execute store result storage exweapons: command.rng float 0.001 run scoreboard players get @s ExWeapons.DetailAbility.MovementSpeed
...
data merge storage exweapons: {command:{ability:["attribute ","player_name"," generic.movement_speed modifier add 70-1-9CB1-0-1 Ability_MovementSpeedMul ","0","0","0",".","0","0","0"," multiply"]}}
...
scoreboard players operation $CommandPosition ExWeapons.rng = $CommandPositionSet_MovementSpeed ExWeapons.rng
