...
## 記録
scoreboard players add $CommandPositionSet_AttackDamage ExWeapons.rng 1
## リセット
attribute @s generic.attack_damage modifier remove 6C-1-9CB1-0-1
## 結合-計算
execute store result storage exweapons: command.rng float 0.001 run scoreboard players get @s ExWeapons.DetailAbility.AttackDamageMul
...
data merge storage exweapons: {command:{ability:["attribute ","player_name"," generic.attack_damage modifier add 6C-1-9CB1-0-1 Ability_AttackDamageMul ","0","0","0",".","0","0","0"," multiply"]}}
...
scoreboard players operation $CommandPosition ExWeapons.rng = $CommandPositionSet_AttackDamage ExWeapons.rng
