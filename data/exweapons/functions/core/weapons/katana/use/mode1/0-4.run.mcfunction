#スラッシュ
## 攻撃
 execute as @s at @s run function exweapons:core/weapons/katana/use/mode1/1-4.tp
 execute as @s at @s run function exweapons:core/weapons/katana/use/mode1/2-4.damage
## 処理
 scoreboard players add @s ExWeapons.katana.useReturn 2
...
### Compound-Nbt
 ...
 data merge storage exweapons: {item:{weapon:{nbt:{ModeAttack:4}}}}
