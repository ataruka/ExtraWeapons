# 斬撃のダメージ処理
 ...
 execute anchored eyes positioned ^ ^-0.5 ^ run function exweapons:core/particle/katana/mode1/7
 ...
 scoreboard players remove $Tmp_AdDamage ExWeapons.rng 60
 execute if score $Tmp_AdDamage ExWeapons.rng matches 80.. run scoreboard players set $Tmp_AdDamage ExWeapons.rng 80
 scoreboard players operation $Tmp_Multiplier ExWeapons.rng = $katana_useMultiplier_Mode1_7 ExWeapons.setting
 scoreboard players operation $Tmp_Multiplier ExWeapons.rng += $Tmp_AdDamage ExWeapons.rng
 scoreboard players operation $Tmp ExWeapons.rng *= $Tmp_Multiplier ExWeapons.rng
...
## ダメージ適用
### Player 
 execute if entity @e[tag=tmp.owner,type=player] if score $FriendlyFire ExWeapons.setting matches 0 anchored eyes positioned ^ ^ ^ run tag @e[distance=..3,type=!#exweapons:non_living,tag=!exweapons.protect,type=!#exweapons:protect,tag=!tmp.owner,type=!player] add tmp.target
 execute if entity @e[tag=tmp.owner,type=player] if score $FriendlyFire ExWeapons.setting matches 1 anchored eyes positioned ^ ^ ^ run tag @e[distance=..3,type=!#exweapons:non_living,tag=!exweapons.protect,type=!#exweapons:protect,tag=!tmp.owner] add tmp.target
### Mob
 execute if entity @e[tag=tmp.owner,type=!player] anchored feet positioned ^ ^ ^ run tag @e[distance=..5,type=!#exweapons:non_living,tag=!exweapons.protect,type=!#exweapons:protect,tag=!tmp.owner] add tmp.target
### Run
...
### Mob
 execute if entity @e[tag=tmp.owner,type=!player] anchored feet positioned ^ ^ ^ run tag @e[distance=..5,type=!#exweapons:non_living,tag=!exweapons.protect,tag=!tmp.owner] add tmp.target
### Run
 ...
 execute as @e[tag=tmp.target] run function exweapons:core/abnormal/wind/damage_lv2
 execute as @e[tag=tmp.target] run function exweapons:core/abnormal/wind/spread_lv3
