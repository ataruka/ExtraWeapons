# LimitOut処理
 ...
 summon area_effect_cloud ~ ~0.5 ~ {Tags:["ExWeapons.AEC"],Radius:0f,Duration:6,DurationOnUse:0f,Age:4,effects:[{id:"resistance",amplifier:9b,duration:20,show_particles:1b}]}
 
 execute rotated ~-135 0 positioned ^ ^ ^6 run particle minecraft:dust 1 1 0 2 ~ ~1 ~ 1 1 1 1 500 force
 execute rotated ~-135 0 positioned ^ ^ ^6 summon minecraft:lightning_bolt run tp @s ~ ~-100 ~
 execute rotated ~-135 0 positioned ^ ^ ^6 run tag @e[type=!#exweapons:non_living,tag=!exweapons.protect,distance=..5] add ExWeapons.Abnormal.thunderLv1
 execute rotated ~-135 0 positioned ^ ^ ^6 run scoreboard players set @e[type=!#exweapons:non_living,tag=!exweapons.protect,distance=..5] ExWeapons.Abnormal.thunderTick 400
# LimitOutのダメージ処理
 tag @s add tmp.owner
 ...
 execute if entity @e[tag=tmp.owner,type=player] if score $FriendlyFire ExWeapons.setting matches 0 rotated ~-135 0 positioned ^ ^ ^6 run tag @e[distance=..5,type=!#exweapons:non_living,tag=!exweapons.protect,type=!#exweapons:protect,tag=!tmp.owner,type=!player] add tmp.target
 execute if entity @e[tag=tmp.owner,type=player] if score $FriendlyFire ExWeapons.setting matches 1 rotated ~-135 0 positioned ^ ^ ^6 run tag @e[distance=..5,type=!#exweapons:non_living,tag=!exweapons.protect,type=!#exweapons:protect,tag=!tmp.owner] add tmp.target
 execute if entity @e[tag=tmp.owner,type=!player] rotated ~-135 0 positioned ^ ^ ^6 run tag @e[distance=..5,type=!#exweapons:non_living,tag=!exweapons.protect,type=!#exweapons:protect,tag=!tmp.owner] add tmp.target
