...
### 特殊
 execute store result score $Tmp2 ExWeapons.rng run scoreboard players get @s OhMyDatID
 execute positioned ^ ^ ^4 as @e[distance=..9,nbt={HurtTime:20s}] at @s on attacker if entity @s[tag=tmp.owner] run tag @e[sort=nearest,limit=1] add tmp.target
 scoreboard players reset $Tmp2 ExWeapons.rng
### Run
